...
class TestPlatformStealthBranch:
    ...
    async def test_xhs_stealth_branch(self):
        ...
        with patch.object(
            ...
            # Raise after stealth call to stop further execution
            mock_browser_context.new_page = AsyncMock(side_effect=StopAsyncIteration("test stop"))
            try:
                await crawler.start()
    ...
    async def test_xhs_cdp_branch_when_stealth_disabled(self):
        ...
        with patch.object(
            ...
            with patch('media_platform.xhs.core.async_playwright') as mock_playwright_cm:
                mock_playwright = AsyncMock()
                mock_playwright_cm.return_value.__aenter__ = AsyncMock(return_value=mock_playwright)
                mock_playwright_cm.return_value.__aexit__ = AsyncMock(return_value=False)
                # Stop execution after CDP call
                mock_browser_context.new_page = AsyncMock(side_effect=StopAsyncIteration("test stop"))
                try:
                    await crawler.start()
                except (StopAsyncIteration, Exception):
    ...
    @patch('config.ENABLE_IP_PROXY', False)
    @patch('config.HEADLESS', True)
    async def test_xhs_standard_branch(self):
...
class TestEdgeCases:
    ...
    @patch('cloakbrowser.launch_persistent_context_async', new_callable=AsyncMock)
    @patch('cloakbrowser.launch_context_async', new_callable=AsyncMock)
    async def test_user_data_dir_contains_platform(
        self, mock_launch_context, mock_launch_persistent
    ):
        """When SAVE_LOGIN_STATE=True, user_data_dir should contain the platform name."""
        crawler = ConcreteCrawler()
        await crawler.launch_browser_stealth(
            playwright_proxy=None,
            user_agent=None,
        ...
        call_kwargs = mock_launch_persistent.call_args[1]
        assert "xhs" in call_kwargs["user_data_dir"]
        assert "browser_data" in call_kwargs["user_data_dir"]

    @pytest.mark.asyncio
    @patch('config.SAVE_LOGIN_STATE', False)
    @patch('cloakbrowser.launch_context_async', new_callable=AsyncMock)
    @patch('cloakbrowser.launch_persistent_context_async', new_callable=AsyncMock)
    async def test_viewport_always_1920x1080(self, mock_launch_persistent, mock_launch_context):
        """Viewport should always be set to 1920x1080."""
        crawler = ConcreteCrawler()
        await crawler.launch_browser_stealth(
            playwright_proxy=None,
            user_agent=None,
